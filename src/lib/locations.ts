...
export default class Locations {
      ...
      try {
        ...
        if (this.users.size < 1) {
          ...
          logger.log(
            Severity.INFO,
            {},
            "ESI is offline, waiting 35 seconds to check again"
          );
          await this.sleep(35);
        }
      } catch (error) {
        ...
        console.info(
          "Location service encountered an error, waiting 15 seconds before running next instance"
        );
        await this.sleep(15);
      }
    }
  ...
  private processUsers = async () => {
    await map(this.users, user => this.processUser(user[1]), {
      concurrency: 500
    });
  };

  private processUser = (user: database.DataSnapshot) =>
    new Promise(async (resolve, reject) => {
      ...
      if ("error" in login) {
        ...
        "id" in login &&
        login.sso &&
        this.hasLocationScopes(login.sso)
      ) {
  ...
  private processOnlineCharacter = async (
    character: Character
  ): Promise<void> => {
  ...
  private setCharacterLocation = async (
    character: Character,
    results: (Location | Ship | ErrorResponse)[]
  ): Promise<void> => {
    let ship: Ship;
